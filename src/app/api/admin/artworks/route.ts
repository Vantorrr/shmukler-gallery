...
function normalizeArtworkData(data: any) {
  return {
    ...data,
    artistsJson: data.artistsJson || null,
    variantMode: data.variantMode || 'single',
    relatedArtworkIds: Array.isArray(data.relatedArtworkIds)
      ? stringifyStringArray(data.relatedArtworkIds)
      : (data.relatedArtworkIds || '[]'),
